...
python3 src/convert_xlsx_into_tsv.py \
    input/gene_list_trusight_pan_cancer.xlsx \
    input/formatted_gene_list_trusight_pan_cancer.tsv
...
TEST_DIRECTORY_IN_TRUSIGHT=$(dx cat file-GpGVQZj4JJKy6vGyZz2GQ2p2 | \
    grep -F -f input/formatted_gene_list_trusight_pan_cancer.tsv -w -i | \
    wc -l)
echo "TruSight recognises ${TEST_DIRECTORY_IN_TRUSIGHT} gene fusion of the NHS Test Directory"
...
TEST_DIRECTORY_NOTIN_TRUSIGHT=$(dx cat file-GpGVQZj4JJKy6vGyZz2GQ2p2 | \
    grep -F -f input/formatted_gene_list_trusight_pan_cancer.tsv -w -i -v | \
    wc -l)
echo "TruSight does not recognise ${TEST_DIRECTORY_NOTIN_TRUSIGHT} gene fusion of the NHS Test Directory"
