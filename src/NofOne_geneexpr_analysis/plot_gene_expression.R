...
if (length(args) == 0) {
  stop("Please provide a gene name, a log2-transformed dense matrix, and an upper threshold for outliers as an arguments")
}

gene_of_interest <- args[1]
gene_of_interest_symbol <- args[2]
log2_dens_file <- args[3]
upper_thresh <- args[4]
prefix <- args[5] #tumour or control

# ---------------------------
# Load data
# ---------------------------
log2_dense <- readRDS(log2_dens_file)
upper_thresh <- fread(upper_thresh, header = FALSE)
...
# ---------------------------
ggsave(
  filename = paste0("./output/", prefix, "_", gene_of_interest_symbol,"_", gene_of_interest, "_scatter_outliers.png"),
  plot = p,
  width = max(5, 100 * 0.2),
  height = 10,
)